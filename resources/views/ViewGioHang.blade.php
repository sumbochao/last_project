                            ...
                            @foreach (Cart::content() as $contentValue)
                                ...
                                <tr>
                                    <td class="li-product-remove"><a
                                            href="{{route('deleteSanPhamKhoiGioHang',['rowId'=>$contentValue->rowId])}}"><i
                                                class="fa fa-times"></i></a></td>
                                    <td class="li-product-thumbnail"><a href="#"><img
                                                src="{{asset('storage/'.$contentValue->options['image'])}}"
                                                width="150px" height="150px" alt="Li's Product Image"></a></td>
                                    <td class="li-product-name"><a
                                            href="{{route('viewChiTietSanPham',['id'=>$contentValue->id])}}">{{$contentValue->name}}</a>
                                    </td>
                                    <td class="li-product-price"><span class="amount">{{$contentValue->price}} $</span>
                                    </td>
                                    <td class="quantity">
                                        <form action="{{route('updateQualtityCart')}}" method="post"
                                              class="cart_quantity_updates">
                                            @csrf
                                            <label>Quantity</label>
                                            <div class="cart-plus-minus">
                                                <input class="cart-plus-minus-box" name="soLuong"
                                                       value="{{$contentValue->qty}}" type="text">
